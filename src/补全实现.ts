...
export function 注册已知语言补全器(context: vsc.ExtensionContext, 语言: string, 触发字符: string[], 未配置 = false) {
    vsc.debug(`注册${未配置 ? '通用' : ''}补全器：${语言}, 触发字符：${触发字符}`);
...
export function 注册未知语言补全器(context: vsc.ExtensionContext) {
    vsc.debug(`注册未知语言补全器：*，触发字符：${通用语言实现.触发字符}`);
    context.subscriptions.push(
        vsc.languages.registerCompletionItemProvider(
            { language: '*' },
...
export async function 补全实现(
    ...
    const 锚点 = 语言.需要矫正补全锚点(文档) ? vsc.矫正补全锚点(文档, 位置) : 位置;

    vsc.debug(`补全「${输入值}」${文档.fileName}, ${文档.languageId}, ${锚点.line}:${锚点.character}`);
    ...
    // for (const 补全项 of 系统补全列表) { vsc.debug(`系统补全项：${JSON.stringify(补全项)}`); }

    const 补全列表 = 语言.生成中文补全(env.编码器, 系统补全列表, 输入值);

    // for (const 补全项 of 补全列表) { vsc.debug(`中文补全项：${JSON.stringify(补全项)}`); }
