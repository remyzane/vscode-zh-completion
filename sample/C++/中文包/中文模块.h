...
// 枚举定义
enum class 颜色枚举 { 红色 = 1, 绿色 = 2, 蓝色 = 3 };
...
public:
  ...
  virtual void 发出声音() { std::cout << 名称 << "发出了声音" << std::endl; }
...
public:
  ...
  void 发出声音() override { std::cout << 名称 << "汪汪叫" << std::endl; }
};

// 除数不能为零异常类
class 除数不能为零异常 : public std::exception {
public:
  const char *what() const noexcept override { return "错误：除数不能为零"; }
};
...
// 模板函数示例
template <typename T> T 最大值(const T &a, const T &b) {
  return (a > b) ? a : b;
}
